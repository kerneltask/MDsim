...
spec = [
    ...
    # cached values that rely on large rij calculation
    ('_F_cache', float64[:,:]),
    ('_U_cache', float64),
    ...
    # verlet list
    ('skin', float64),                          # verlet skin thickness (0.2 recommended)
    ('verlet_r_squared', float64),              # verlet radius (2.7^2 recommended)
    ('verlet_list', int64[:, :]),               # neighbor indices (n x n)
    ('verlet_counts', int64[:]),                # number of neighbors for each particle (n)
    ('last_verlet_positions', float64[:, :]),   # positions of particles on last verlet build
    ('num_verlet_builds', int64),               # number of times verlet lists are built
...
class ParticleSystem:
    def __init__(self, positions, velocities, r_cut, temp_guess, temp_des,
             ...
             record_freq, thermostat_damping, verlet_skin):

        n_py, d_py = positions.shape
        n = int(n_py)
        d = int(d_py)
        ...
        steps_py = int(num_steps)
        rec_py = int(max(1, record_freq))
        self.steps = steps_py
        self.record_stride = rec_py
        frames_py = (steps_py + rec_py - 1) // rec_py
        self.frames = frames_py
        self.frame = 0
        self.temp_des = float(temp_des)
        self.thermo_off = thermo_off_timestep
        self.eqbm_step = eqbm_timestep
        self.tau_damp = float(thermostat_damping)
        self.zeta = self.tau_damp
        self.post_eqbm_frames = max(1, (steps_py - int(self.eqbm_step)) // rec_py)

        # integer wrap counts and unrolled
        self.wrap_count = np.zeros((n, d), dtype=np.int64)
        self.unrolled = self.xi.copy()

        # recording arrays (use Python ints for shape arguments)
        f = int(self.frames)
        self.positions = np.empty((n, d, f), dtype=np.float64)
        self.velocities = np.empty((f, n, d), dtype=np.float64)
        self.wrap_history = np.empty((n, d, f), dtype=np.int64)
        self.unrolled_array = np.empty((n, d, f), dtype=np.float64)

        self.K_array = np.empty(f, dtype=np.float64)
        self.U_array = np.empty(f, dtype=np.float64)
        self.total_energy_array = np.empty(f, dtype=np.float64)
        self.momentum_array = np.empty((self.d, f), dtype=np.float64)
        self.temperature_array = np.empty(f, dtype=np.float64)
        self.pressure_array = np.empty(f, dtype=np.float64)
        self.MSD_array = np.empty(f, dtype=np.float64)
        self.COM_array = np.empty((int(self.d), f), dtype=np.float64)

        self.time_average_K = np.empty(f, dtype=np.float64)
        self.time_average_U = np.empty(f, dtype=np.float64)
        self.time_average_temperature = np.empty(f, dtype=np.float64)
        self.time_average_pressure = np.empty(f, dtype=np.float64)
        ...
        self._F_cache = np.zeros((n, d), dtype=np.float64)
        self._U_cache = 0.0
        ...
        self.last_verlet_positions = self.unrolled.copy()
        self.num_verlet_builds = 0

        # build initial verlet list
        ...
        self.momentum = np.zeros(int(self.d), dtype=np.float64)
        self.MSD = 0.0
        self.MSD_slope = 0.0
        self.MSD_intercept = 0.0
        self.center_of_mass = np.zeros(int(self.d), dtype=np.float64)
    ...
    def compute_forces_and_energy(self):
        if self._cache_step != self.i:
            # check if verlet list needs to be rebuilt
            self.check_update_verlet()
            # calculate pairwise forces/energy
    ...
    def build_verlet_list(self):
        n = self.n
        d = self.d
        # reset counts and list
        self.verlet_counts.fill(0)
        self.verlet_list.fill(-1)

        xi = self.xi # current positions (n x d)
        for i in range(n):
            for j in range(i + 1, n): # perform on upper diagonal (array is symmetric with no self-interaction)
                # calculate pairwise distances
                r2 = 0.0
                for dim in range(d):
                    dim_dist = xi[i, dim] - xi[j, dim]
                    dim_dist -= self.L * np.round(dim_dist / self.L)
                    r2 += dim_dist * dim_dist
                if r2 < self.verlet_r_squared: # pair is within verlet radius
                    # make contiguous rows in verlet_list to avoid if loops in future rij calcs
                    ci = self.verlet_counts[i]
                    cj = self.verlet_counts[j]
                    # j is a neighbor of i
                    self.verlet_list[i, ci] = j
                    self.verlet_counts[i] = ci + 1
                    # i is a neighbor of j
                    self.verlet_list[j, cj] = i
                    self.verlet_counts[j] = cj + 1

        # update last_positions to current unwrapped positions
        self.last_verlet_positions = self.unrolled.copy()
        self.num_verlet_builds += 1

    def check_update_verlet(self):
        # check maximum displacement since last build
        self.unrolled = self.xi + self.wrap_count * self.L # !note: this is not computed every step in the non-verlet list MD simulation, could be a source of slow down
        x_now = self.unrolled
        x_last = self.last_verlet_positions
        ...
        for i in range(self.n):
            ...
            for dim in range(self.d):
                diff = x_now[i, dim] - x_last[i, dim]
                disp2 += diff * diff
            if disp2 > max_disp2:
                max_disp2 = disp2

        thresh2 = (0.5 * self.skin) ** 2
        if max_disp2 > thresh2:
            self.build_verlet_list()

    def forces_and_energy(self, xi):
        # calculate forces and energy using verlet lists
        n = self.n
        d = self.d

        # reset cache
        self._F_cache.fill(0.0)
        U_accum = 0.0
        virial = 0.0
        
        for i in range(n):
            for idx in range(self.verlet_counts[i]):
                j = self.verlet_list[i, idx]
                if j <= i:
                    continue
                # NIC
                rij = np.empty(d, dtype=np.float64)
                for dim in range(d):
                    rij[dim] = xi[i, dim] - xi[j, dim]
                    rij[dim] -= self.L * np.round(rij[dim] / self.L)
                r2 = 0.0
                for dim in range(d):
                    r2 += rij[dim] * rij[dim]
                if r2 == 0.0:
                ...
                if r < self.r_cut:
                    ...
                    # LJ force magnitude
                    F_mag_raw = 24.0 * inv_r * (2.0 * r6 * r6 - r6)
                    F_mag = F_mag_raw - self.dU_r_cut

                    # apply force to i and -force to j
                    for dim in range(d):
                        F_comp = (F_mag * rij[dim]) / r
                        self._F_cache[i, dim] += F_comp
                        self._F_cache[j, dim] -= F_comp
                        virial += rij[dim] * F_comp
        # totals
        self._U_cache = U_accum
        self._virial_cache = virial
    ...
    def get_center_of_mass(self):
        ...
        com = np.zeros(int(self.d), dtype=np.float64)
...
def main():
    ...
    system = ParticleSystem(test_positions, test_velocities, r_cut, 
                            ...
                            record_freq, tau_damp, skin_thickness)
